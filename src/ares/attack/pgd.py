...
Module to create an interface for the PGD adversarial attack. Based on the BatchAttack base class.
"""
...
class PGD(BatchAttack):
    """Implements the Projected Gradient Descent (PGD) adversarial attack. Works as a white-box iterative
    ...
    def batch_attack(self, xs, ys=None, ts=None):
        ...
        for it in range(1, self.iteration+1):
            # For the first iteration, the adversarial image is randomly generated inside the 'l_2' or 'l_inf' norm
            # ball, depending on the distance metric used. Then, the gradient with respect to the previously crafted
            # adversarial example is used to create the following examples.
            if it == 1:
                adv_delta = torch.rand(xs_og.size())
                adv_dist = torch.rand(self.eff_batch_size)
                if self.distance_metric == "l_2":
                    adv_norm = torch.linalg.vector_norm(adv_delta.flatten(start_dim=1), ord=2, dim=1)
                else:
                    adv_norm = torch.linalg.vector_norm(adv_delta.flatten(start_dim=1), ord=torch.inf, dim=1)

                for i in range(self.eff_batch_size):
                    adv_delta[i] = adv_dist[i] * self.eps[i] * adv_delta[i] / adv_norm[i]

                xs_adv = xs_og + adv_delta
            ...
            if self.iteration_callback:
                ...
                self.pgd[iter_key] = xs_adv.clone()

        self.pgd["xs_adv"] = xs_adv
        return self.pgd
