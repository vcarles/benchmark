...
Module to create an interface for the SPSA adversarial attack. Based on the Attack base class.
"""
...
class SPSA(Attack):
    """Implements the Simultaneous Perturbation Stochastic Approximation (SPSA) adversarial attack. Works as a black-box
    iterative constraint-based method. Requires a classifier model and a differentiable loss function.
    ...
    def attack(self, x, y=None, t=None):
        ...
        x_adv = torch.zeros_like(x).to("cpu")

        # Stores the input tensor for the maximum distortion value.
        x_og = x.detach().clone().to(self.device)

        # Initialize the perturbation value and its gradient.
        pert = torch.zeros_like(x).to(self.device)
        pert.requires_grad_()
        pert.grad = torch.zeros_like(x)

        # Optimizer to generate the perturbation.
        optim = torch.optim.Adam([pert], lr=self.learning_rate)
        ...
        for it in range(1, self.iteration_steps+1):
            ...
            pert.grad = self.grad_sign * self._grad(x_og + pert, y, t)
            optim.step()

            pert = norm_clamp(pert, x_og, x_og, self.distance_metric, self.eps)

            x_adv = x_og.to("cpu") + pert.to("cpu")
            x_adv = torch.clamp(x_adv, self.model.x_min, self.model.x_max)

            if self.iteration_callback:
                iter_key = "x_adv_{}".format(it)
                self.nes[iter_key] = x_adv

        self.nes["x_adv"] = x_adv
        return self.nes

    def _grad(self, x, y, t):
        """Estimate the gradient of the classifier model.
        """
        ...
        ber = torch.zeros_like(x)

        for i in range(self.samples):
            ber = torch.bernoulli(torch.clamp(grad, 0., 1.)).to(self.device)
            ber = 2.0 * ber - 1.0

            with torch.no_grad():
                logits_plus, _ = self.model((x + self.sigma * ber).reshape((1,) + x.shape))
                logits_minus, _ = self.model((x - self.sigma * ber).reshape((1,) + x.shape))

            p_plus = self._loss(logits_plus[0], y, t)
            p_minus = self._loss(logits_minus[0], y, t)
            grad = grad + (p_plus - p_minus) * ber
